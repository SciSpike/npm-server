...
NpmProxy.prototype.auth = function (req, res) {
  var address = req.connection.remoteAddress || req.socket.remoteAddress,
      method = req.method.toLowerCase(),
      host,
      npm;

  npm = this.policy.auth;

  host = npm.vhost || npm.hostname;

  this.log.info('[auth] %s - %s %s %s %j', address, req.method, req.url, host, req.headers);
  req.headers.host = host;
...
NpmProxy.prototype.private = function (req, res, policy) {
  ...
  console.log('[private] %s - %s %s %s %j', address, req.method, req.url, host, req.headers);
  req.headers.host = host;
...
NpmProxy.prototype.decide = function (req, res, policy) {
  ...
  function onDecision(err, target) {
    ...
    //
    self.log.info('[decide] %s - %s %s %s %j', target.href, req.method, req.url, target.vhost || target.hostname, req.headers);
    if(!target.secure){
      delete req.headers.authorization
    } else {
      req.headers["user-agent"] = "secure/"+ req.headers["user-agent"]
    }
...
NpmProxy.prototype.notFound = function (req, res, err) {
  var address = req.connection.remoteAddress || req.socket.remoteAddress,
  port = req.connection.remotePort || req.socket.remotePort,
  lport = req.connection.localPort || req.socket.localPort,
      code    = err ? 400 : 404,
      json;

  if (!err) { global.console.trace(); }
  err = err || { message: 'Unknown error' };
  this.log.error('[not found] %s :%s:%s - %s %s %s %j', address, port, lport, req.method, req.url, err.message, req.headers);
