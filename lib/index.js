...
exports.createRouter = function createRouter(options, callback) {
  ...
  rewrites = {
    ...
    user: options.rewrites.filter(function (rr) {
      return /_users/.test(rr.to)|| /^\/-\/user/.test(rr.from);
    }),
    session: options.rewrites.filter(function (rr) {
      return /_session/.test(rr.to);
    })
  };
  ...
  rewrites.user.forEach(function (rr) {
    ...
    log.info('[route]', '%s - %s %s', 'auth', method, from);
    router[method](from, function () {
      proxy.auth(this.req, this.res);
    });
  });
  rewrites.session.forEach(function (rr) {
    var method = rr.method.toLowerCase(),
        from   = rr.from;
